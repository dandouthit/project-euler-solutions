import operator
import time

start = time.time()

def prod(iterable):
    return reduce(operator.mul, iterable, 1)

def calc_prod(slice):
    int_list = map(int,slice)
    int_prod = prod(int_list)
    return int_prod

big_number = 

big_str = str(big_number)

# initialize variables
zero_index = []
biggest_product = 0

# find all of the zeros in the "big number"
for i, char in enumerate(big_str):
    if char == '0':
        zero_index.append(i)

# grab slice of big string between each zero position
for num in zero_index:
    # special case for the first slice
    if zero_index.index(num) == 0:
        if num > 12:
            # need to generalize for case where first slice is not exactly 13 characters
            k = 0
            while k + 13 <= num:
                slice = big_str[:num]
                int_prod = calc_prod(slice)
                if int_prod > biggest_product:
                    biggest_product = int_prod
                    print "Biggest product is now: " + str(biggest_product)
                k = k + 1

    elif (num - zero_index[zero_index.index(num) - 1]) > 12:
        j = zero_index[zero_index.index(num) - 1]
        # stop slice loop once the next zero is reached
        while j + 13 < num:  
            slice = big_str[j+1:j+14]
            int_prod = calc_prod(slice)
            if int_prod > biggest_product:
                biggest_product = int_prod
                print "Biggest product is now: " + str(biggest_product)
            # increment j counter to "move" the slice
            j = j + 1
        # print "The difference between " + str(num) + " and " + str(zero_index[zero_index.index(num) - 1]) + " is 13 or greater!"

elapsed = (time.time() - start)
print "This code took " + str(elapsed) + " seconds to run."
